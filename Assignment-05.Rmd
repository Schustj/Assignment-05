...
data2 <- raw_data %>% pivot_longer(-CNTRYNAME, names_to = "Date", values_to = "Total_Deaths") %>% separate(col = "Date", into = c("Month", "Day"), sep = 2) %>% mutate(Date = mdy(paste(Month, Day, "2020", sep = "-"))) %>% filter(CNTRYNAME %in% c("France", "Germany", "Italy")) %>% rename(Country = `CNTRYNAME`) %>% lag()
...
ggplot(data2, aes(Country, Total_Deaths), fill = "Country") + geom_line(size = 1.5) + scale_y_log10() + labs(y = "Daily COVID-19 Deaths") + theme_linedraw()
