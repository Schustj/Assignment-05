...
data2 <- raw_data %>% pivot_longer(-CNTRYNAME, names_to = "Date", values_to = "Daily_Deaths") %>% separate(col = "Date", into = c("Month", "Day"), sep = 2) %>% mutate(Date = mdy(paste(Month, Day, "2020", sep = "-"))) %>% filter(CNTRYNAME %in% c("France", "Germany", "Italy")) %>% rename(Country = `CNTRYNAME`) %>% lag(1)
...
ggplot(data1, aes(Country, Total_Deaths), fill = "Country") + geom_line(size = 1.5) + scale_y_log10() + labs(y = "Total COVID-19 Deaths") + theme_linedraw()
...
ggplot(data2, aes(Country, Daily_Deaths), fill = "Country") + geom_line(size = 1.5) + scale_y_log10() + labs(y = "Daily COVID-19 Deaths") + theme_linedraw()
...
knitr::kable(data2 %>% group_by(Daily_Deaths) %>% mutate(Max_Deaths = max(Daily_Deaths)))
